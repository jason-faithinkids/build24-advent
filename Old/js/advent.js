...
function initialiseAdvent() {
    var now = new Date();
    var unlockedMonths = [10, 11]; // November (10) and December (11)
    var isLiveWindow = unlockedMonths.indexOf(now.getMonth()) !== -1;
    var today = isLiveWindow ? now.getDate() : 0;
    ...
    dayElements.forEach(function(element) {
        ...
        if (!isLiveWindow || dayNumber > today) {
            // If we're outside the unlock window or the day is in the future, make it inactive
